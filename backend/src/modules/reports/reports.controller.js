...
export async function exportExcel(req, res, next) {
  try {
    const { year, month, startDate, endDate } = req.query;
    const buffer = await reportsGenerator.generateExcelReport({ year, month, startDate, endDate });
...
export async function exportPdf(req, res, next) {
  try {
    const { year, month, startDate, endDate } = req.query;
    const buffer = await reportsGenerator.generatePdfReport({ year, month, startDate, endDate });
...
export async function exportDocx(req, res, next) {
  try {
    const { year, month, startDate, endDate } = req.query;
    const buffer = await reportsGenerator.generateDocxReport({ year, month, startDate, endDate });
